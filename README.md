...
`It connects the actor to an individual film`

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?

`the blue diamond indicates that the value in that key cannot be NULL, if it was an empty blue diamond it means that the value can potentially be NULL`

3. How many foreign keys does the "payments" table have? How can you tell?

`there are 3 foreign keys, I can tell due to the fact that each of those keys are stated in another table as a primary key, and that they are all red to help identify them better.`
